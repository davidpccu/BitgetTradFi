...
- **設計建議**：不可依賴容器記憶體或本機磁碟保存去重與對沖狀態；使用受管理 PostgreSQL（交易資料）及選配 Redis（快取／租約），所有必要狀態以 DB 為準。
- **設計建議**：MVP 固定一個 active trader。即使 Web Service 被擴成多 replica，也以 DB advisory lock／lease 選出唯一 worker；非 leader 只服務唯讀 UI。租約失效時先進入 `RECOVERY`，不可直接接手下單。
...
所有核心數量以 **base token** 表示，使用 decimal library／資料庫 `NUMERIC`，禁止 JavaScript `number`。
...
3. 以 `tradeId` 去重；若只有累計成交量，鎖定 order row 並計算 `max(exchangeCumFilled - processedCumFilled, 0)`。累計量倒退或同版本內容衝突時轉 `RECOVERY`。
4. 在同一 DB transaction 寫入 raw event、正規化 fill、更新累計量、建立／更新 hedge intent 與 outbox；commit 後才允許 worker 送單。
...
| 資料表 | 關鍵欄位／約束 |
|---|---|
| `exchange_events` | raw payload（已遮罩）、channel、receivedAt；`UNIQUE(exchange, account, eventIdentity)` |
| `spot_fills` | tradeId/orderId、side、baseQty、price、feeAsset/feeQty、exchangeTime；tradeId 唯一 |
| `spot_orders` | orderId、clientOid、lastCumFilled、lastVersion、人工／自有分類 |
| `hedge_intents` | deterministic intentId、target signed qty、quantized qty、status、deadline、configVersion |
| `hedge_orders` | intentId、clientOid、orderId、requestedQty、cumFilled、status；clientOid 唯一 |
| `futures_fills` | tradeId/orderId、signedQty、price、fee、exchangeTime；tradeId 唯一 |
| `strategy_state` | mode、lease fencing token、last reconciled cursor/time、last error |
| `config_versions` | 參數、effectiveAt、actor、approval metadata；不可原地覆寫 |
| `audit_log` | actor、action、before/after、requestId、timestamp；append-only |

`clientOid` 建議由 `{prefix}{env}_{intentId}_{attempt}` 產生。網路 timeout 後，先以相同 `clientOid` 查 order/open orders/fills；只有交易所明確證實不存在，才依官方冪等契約決定沿用原 ID 或增加 attempt。任何「查不到但查詢本身不完整」都是 `UNCERTAIN`，不是可重送證據。
...
| 重複／亂序 | tradeId unique + order row lock + cumulative high-water mark | 累計倒退、版本衝突或無法補 gap |
...
| DB 不可用／失去 leader | fail closed，停止送單；現有請求以 fencing token 防舊 leader commit | 一律進 recovery |
| 時鐘漂移 | NTP 監控、簽章時間錯誤時停止私有操作 | 漂移超限 |
| kill switch | 下單 adapter 最內層再次檢查，拒絕 place/replace | 一直維持至雙人或明確授權解除 |

### 啟動／重啟 runbook

1. 啟動為 `RECOVERY`，讀取 secrets 與已簽署／版本化設定；未取得 DB 與唯一 leader lease 時 fail closed。
...
5. 對每個自有 `clientOid` 重建 requested、working、filled；對 Spot tradeId 重建人工 fills。資料落庫仍走 unique constraints。
6. 比對 DB 推導 Futures fills、REST orders/fills 與實際 one-way position。差異不能合理解釋時進 `PROTECTED`，不得自動以新單抹平。
...
- WS stale／sequence gap、REST 429、order uncertain、position mismatch、leader churn、DB failure、instrument/session 變更。
...
首選將管理端點置於 Zero Trust access proxy／私有網路或獨立的 Koyeb private exposure；若平台無法保證網路層不可達，MVP 可先**不提供遠端管理 route**，以受控部署／DB 管理程序設定 kill，直到完成強身份驗證。僅「URL 難猜」或共用 bearer token 不合格。
...
| 單元 | 同 tradeId 重播 | DB unique 命中，目標與送單次數不變 |
...
| 整合 | 程序在 DB commit／HTTP 前後逐點崩潰 | outbox replay；每個 intent 不遺失且不重複曝險 |
| 整合 | 兩 replica 同時成 leader | fencing 後只有一個可送單 |
| 對帳 | DB fill 與實際 position 不符 | 禁止 running，不自動猜測補單，告警人工判讀 |
...
- 單一 Web Service、外部 DB、公開 ticker、私有 WS、raw event archive、唯讀 UI。
...
- 導入 intent/outbox、signed quantity、risk gate、leader fencing、recovery 與所有 fault injection 測試。
...
- PR 必須通過 lint、typecheck、unit/integration、architecture boundary、migration、dependency/secret/container scan。
